...
void CPEFile::SaveExeDump_v2(char * filename)
{
	...
	//	write empty fields. to 
	if (_out.size() < _text_header->PointerToRawData)
	{
		int off = _text_header->PointerToRawData - _out.size();
		for (int i = 0x00; i<off; i++)
		{
			_out.push_back(0);
		}
	}
    ...
    p		= (byte*)&_pe_dot_data_section[0];
     _out.insert(_out.begin()+_out.size(),p,p+size);
    //  save section .rcrs
     _rsrc_header->PointerToRawData = _out.size();
	...
	_pe_nt_header.OptionalHeader.SizeOfImage = 0x01d7000;//_rsrc_header->VirtualAddress + _rsrc_header->Misc.VirtualSize;
    _pe_nt_header.OptionalHeader.CheckSum = 0;
	_pe_nt_header.OptionalHeader.DataDirectory[2].Size = _rsrc_header->Misc.VirtualSize;
	_pe_nt_header.OptionalHeader.DataDirectory[2].VirtualAddress = _rsrc_header->VirtualAddress;
    ...
    //	copy nsis crc
    {
		...
		//crc = NISI_CRC32(crc,&_out[0],_out.size());
        p = (byte*)&crc;
        _out.insert(_out.begin()+_out.size(),p,p+sizeof(DWORD));
...
void CPEFile::ReplaceTextSegment(CPEFile* source)
{
	...
	_pe_nt_header.OptionalHeader.MajorImageVersion = 6;
	_pe_nt_header.OptionalHeader.MinorImageVersion = 0;
	// terminal services aware
	_pe_nt_header.OptionalHeader.DllCharacteristics |= IMAGE_DLLCHARACTERISTICS_TERMINAL_SERVER_AWARE;
	
	//	update nsis codebase
	_eof_dump = source->_eof_dump;
	
	//	copy the resource section
	_pe_dot_rsrc_section = source->_pe_dot_rsrc_section;
	*_rsrc_header= *source->_rsrc_header;
	
	// .ndata header - variable stack
