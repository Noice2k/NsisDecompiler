...
void read_file(const tstring& filename, vector<unsigned char>& data) {
  ...
  if (!file) {
#ifdef _UNICODE
  // Jim Park: No real way to pass back Unicode file names through
  // std::exception.
    throw NSISException("Can't open file.");
#else
  ...
  // Jim Park: No real way to pass back Unicode file names through
  // std::exception.
    throw NSISException("Couldn't read entire file.");
#else
  ...
  if (NTHeaders->FileHeader.Characteristics & IMAGE_FILE_DLL)
  {
    ...
    for (DWORD i = 0; i < num_sections; i++)
    {
          ...
          && va + FIX_ENDIAN_INT32(sections[i].Misc.VirtualSize) >= ExportDirVA + ExportDirSize)
      {
        ...
        {
#ifdef _UNICODE
          // The names of the functions are in ANSi, so we need to convert them to Unicode.
          const string tmpname = string((char*)exports + FIX_ENDIAN_INT32(names[j]) - ExportDirVA);
          WCHAR* wtmp = winchar_fromansi(tmpname.c_str());
          const tstring name(wtmp);
          delete[] wtmp;
#else
